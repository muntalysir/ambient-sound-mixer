...
class AmbientMixer {
  ...
  init() {
    try {
      ...
      // Render sound card using our sound data
      ...
      sounds.forEach((sound) => {
        this.currentSoundState[sound.id] = 0; // default volume 0 (off)
      });
  ...
  setupEventListeners() {
    ...
    document.addEventListener('click', async (e) => {
      ...
      // Check if deleted button was clicked
      if (e.target.closest('.delete-preset')) {
        e.stopPropagation(); // Prevent triggering preset load
        const presetId = e.target.closest('.delete-preset').dataset.preset;
        this.deleteCustomPreset(presetId);

        return; // Exit early to avoid also trying to load the preset
      }
  ...
  // Apply master volume to all playing sound
    ...
    for (const [soundId, audio] of this.soundManager.audioElements) {
      if (!audio.paused) {
        ...
        if (slider) {
          ...
          // Apply to hte actual audio element
          audio.volume = effectiveVolume / 100;
        }
      }
    }
  ...
  showSavePresetModal() {
    ...
    if (!hasActiveSounds) {
      alert('No active sounds for preset. Please turn on some sounds first.');
      return;
    }
  ...
  saveCurrentPreset() {
    ...
    if (this.presetManager.presetNameExists(name)) {
      alert(
        `Preset with the name "${name}" already exists. Please choose a different name.`,
      );
      return;
    }
